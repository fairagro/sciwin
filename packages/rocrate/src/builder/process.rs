...
use std::path::{Path};
use super::metadata::{classify_and_prefix_params, extract_or_prompt_metadata};
use crate::utils::StepTimestamp;
use super::metadata::{generate_id_with_hash, extract_workflow_steps, get_workflow_structure, extract_parts, generate_connections};
use crate::model::{create_container_image, Action};

const DEFAULT_PROCESS_CONFORMS_TO: &[&str] = &[
    "https://w3id.org/ro/crate/1.1",
    "https://w3id.org/workflowhub/workflow-ro-crate/1.0",
    "https://w3id.org/ro/wfrun/process/0.5",
];

pub async fn create_ro_crate_metadata_json_from_graph_workflow_processcrate(
    graph_json: &[Value],
    workflow_toml: &str,
    rocrate_dir: &Path,
    workflow_file: &str,
    ...
    let _connections_slice: &[(String, String, String)] = &connections;
    ...
    let creative_works: Vec<DataEntity> =
        DEFAULT_PROCESS_CONFORMS_TO.iter().map(|id| create_creative_work(id)).collect();
