...
use crate::model::{Action, create_container_image, create_howto_steps, create_action};
...
const DEFAULT_PROVENANCE_CONFORMS_TO: &[&str] = &[
...
pub async fn create_ro_crate_metadata_json_from_graph_provenance_runcrate(
    graph_json: &[Value],
    workflow_toml: &str,
    rocrate_dir: &Path,
    workflow_file: &str,
    ...
    let _workflow_file_path = working_dir.join(workflow_file);
    ...
    let connections_slice: &[(String, String, String)] = &connections;
    let (name, description, license) =
        ...
        Some("./"),
        DEFAULT_PROVENANCE_CONFORMS_TO,
    );
    ...
    let creative_works: Vec<DataEntity> =
        DEFAULT_PROVENANCE_CONFORMS_TO.iter().map(|id| create_creative_work(id)).collect();
    ...
    for step in script_structure.values() {
        ...
        if is_cwl_file(id) {
            ...
            let howto_steps = create_howto_steps(&steps, connections_slice, id, workflow_file);
            graph.extend(howto_steps.into_iter().map(GraphVector::DataEntity));
            ...
            if let Some(step_val) = step_opt {
                let control_action = Action {
                    ...
                    id: &control_id,
                    name: &format!("orchestrate {}", id.trim_start_matches('#')),
                    instrument_id: &format!(
                        "{workflow_file}#main/{}",
                        step_val.trim_start_matches('#').trim_end_matches(".cwl")
                    ),
                ...
                graph.push(GraphVector::DataEntity(create_action(control_action)));
            }
        ...
        .find(|s| s.id == "#main" || s.id == format!("{workflow_file}#main"))
    {
        ...
        let organize_res_str: Vec<&str> = vec![&workflow_create_id];
        ...
        graph.push(GraphVector::DataEntity(create_action(organize_action)));
    }
