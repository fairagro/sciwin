...
fn parse_repo_url(url: &str) -> RepoInfo {
    ...
    // Fallback: assume it's already a repo
    RepoInfo {
...
/// Clone repo from isBasedOn in ro-crate-metadata.json or from s:codeRepository in CWL file
pub fn clone_from_rocrate_or_cwl(
            ...
            if cwl_path.exists() {
                fs::read_to_string(cwl_path).ok().and_then(|content| {
                    content.lines()
                        ...
                        .and_then(|l| l.split_once("s:codeRepository:"))
    ...
    // --- Resolve CWL + inputs ---
    let (cwl_candidate, inputs_yaml_candidate) = if let Some(path) = repo.file_path {
