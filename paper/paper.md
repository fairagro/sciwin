...
CWL was introduced as a vendor-neutral, platform agnostic standard to address fragmentation. CWL workflows are portable by design as they in principle can run on any compliant execution engine. There were even efforts to make Galaxy and Nextflow compliant to this standard [@cwl2nxf;@cwl_galaxy_parser]. However, the adoption of CWL remains lower than that of Nextflow and Galaxy, and the standard provides comparatively little support for workflow creation. Its verbose, YAML-based syntax requires familiarity with structured data formats and workflow abstractions that many domain researchers do not possess. This creates a gap between the portability offered by a standardized workflow representation and the accessibility of authoring such workflows.

The CWL ecosystem further compounds this problem. While a number of great runner implementations exist (e.g. cwltool, Toil [@Vivian_2017], REANA, Arvados)[@CWLImplementations], which are mostly written in Python and however do not support Windows natively (cwltool needs WSL). The space of authoring tools is sparse [@CWLGen]. Rabix offered a graphical editor (Rabix Composer) which was made closed-source and moved into the Seven Bridges Platform  [@Rabix2016]. The open-sourced version has been unmaintained for over 5 years and is significantly outdated. Many generators are outdated as well, meaning there is no actively developed open and lightweight CWL generator that integrates naturally into a researchers existing command-line-driven "workflow". 
...
Workflows can be executed either directly by using CWL files or by using Workflow RO-Crates [@Bacall2026WorkflowROCrate] or Workflow Run RO-Crates [@Leo2024WRRC]. Structured execution results in form of Workflow Run RO-Crates using the Provenance Run Crate profile can be exported for each execution run.
...
SciWIn-Client was presented at the 2nd Conference on Research Data Infrastructure (CoRDI 2025) in the contribution __Easy creation of reproducible computational workflows with SciWIn-Client__ [@krumsieck_2025_17119086;@krumsieck_2025_16735806]. A workshop on SciWIn-Client was subsequently held at the Boosting Biodata Bootcamp 2026 [@antonia-workshop-slides].
