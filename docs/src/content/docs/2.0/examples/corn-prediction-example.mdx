...
slug: 2.0/examples/corn-prediction-example
---
...
Requires a [REANA](https://reanahub.io) instance, with `REANA_SERVER_URL`/`REANA_ACCESS_TOKEN` set (directly, or via a `.env` file):

```bash
s4n execute --engine reana workflows/demo/demo.cwl inputs.yml
```
