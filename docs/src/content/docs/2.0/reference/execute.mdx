...
`<FILE>` doesn't have to be a raw CWL file. It can also be a Workflow or [Run RO-Crate](../concepts/rocrate) produced by an earlier `s4n execute run --rocrate`, given either as a directory or as a `.zip` archive, as long as it contains a `ro-crate-metadata.json` whose `mainEntity` is CWL. This is what makes a published crate directly re-executable: hand someone (or a CI job) the crate, and `s4n execute run my-run.zip` runs it the same way `s4n execute run my-tool.cwl` would, without unpacking it by hand first. The crate doesn't have to be one `s4n` produced locally either; a Workflow RO-Crate downloaded from a registry like [WorkflowHub](https://workflowhub.eu/) works the same way, as long as it satisfies the same `mainEntity`-is-CWL requirement.

### Credentials and `.env`

`REANA_SERVER_URL`/`REANA_ACCESS_TOKEN` and `TES_URL`/`TES_STORAGE`/`TES_TOKEN` are read from the process environment. `s4n` also loads a `.env` file from the current directory if one is present (via [`dotenvy`](https://docs.rs/dotenvy)), so these can be set once in a `.env` file instead of exporting them in every shell session.

## `execute reana`

This command queries or fetches results for workflow runs already submitted to a [REANA](https://reanahub.io) server (via `s4n execute run --engine reana`). It needs the same `REANA_SERVER_URL`/`REANA_ACCESS_TOKEN` environment variables as `execute run --engine reana`.
