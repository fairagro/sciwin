...
```txt frame="terminal" title="Usage"
    ...
    Options:
      ...
      -i, --inputs <INPUTS>                    Force a value to be considered as an input. Repeat to add more,
                                                e.g. -i file1.txt -i file2.txt
          --run-container <RUN_CONTAINER>      Possible container engines: docker, podman, singularity, apptainer
      -o, --outputs <OUTPUTS>                  Force a value to be considered as an output. Repeat to add more,
                                                e.g. -o file1.txt -o file2.txt
      -m, --mount <MOUNT>                      Mounts a directory into the working directory. Repeat to mount
                                                more than one
      -q, --quiet                              Only print errors
          --debug                              Explain in high detail what the CLI is doing internally
          --env <ENV>                          Loads an .env File
      -f, --force                              Overwrites existing CWL File
          --auto-container                     Automatically adds a Docker Requirement based on dependencies
                                                (Python and R supported)
      -h, --help                               Print help
```
...
Rather than setting `--container-image`/`--container-tag` by hand, `--auto-container` resolves a matching pre-built image from the tool's own dependency file (`requirements.txt`, `pyproject.toml`, `DESCRIPTION`) and writes it into the generated CWL file automatically. See [Containers: Getting one without writing a Dockerfile](../concepts/container#getting-one-without-writing-a-dockerfile) for how that resolution works.
