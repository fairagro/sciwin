...
- **`rocrate`** reads, writes, and validates [RO-Crates](https://www.researchobject.org/ro-crate/), including the Provenance Run Crates `s4n execute run --rocrate` produces.
- **`reana`** is the client used when execution targets a remote [REANA](https://reanahub.io) server instead of running locally.
