...
pub fn render<R: FlowchartRenderer>(
    r: &mut R,
    cwl: &Workflow,
    filename: &Path,
    no_defaults: bool,
        ...
        if !no_defaults && let Some(doc) = load_step(step, filename) {
            for input in &doc.get_inputs() {
                    ...
                    && let Some(input_default) = &input.default
                {
                    ...
                    r.node(
                        &node_id,
                        Some(&input_default.to_string()),
                        RenderStyle::Small,
                    );
                    r.edge(
                        &node_id,
                        step.id.as_ref().unwrap(),
                        Some(input.id.as_ref().unwrap()),
                        RenderStyle::Small,
                    );
                }
            }
...
---"#
    ...
    fn begin_graph(&mut self) {
        ...
        self.storage.push(format!(
            "  linkStyle default stroke:{GREEN_DARK},stroke-width: 2px;"
        ));
    }
    ...
    fn begin_cluster(&mut self, id: &str, label: Option<&str>, style: RenderStyle) {
        self.storage
            .push(format!("  subgraph {id}[{}]", label.unwrap_or(id)));
        self.storage.push("    direction TB".to_string());

        match style {
            RenderStyle::Input | RenderStyle::Output => self
                .styles
                .push(format!("  style {id} fill:{GRAY_LIGHT},stroke-width:2px;")),
            _ => unimplemented!(),
        }
    }
    ...
    fn node(&mut self, id: &str, label: Option<&str>, style: RenderStyle) {
        self.storage
            .push(format!("    {id}({})", label.unwrap_or(id)));
...
impl FlowchartRenderer for DotRenderer {
    ...
    fn begin_cluster(&mut self, id: &str, label: Option<&str>, style: RenderStyle) {
        ...
        self.storage
            .push(format!("    label=\"{}\";", label.unwrap_or(id)));
        ...
        if matches!(style, RenderStyle::Input) || matches!(style, RenderStyle::Output) {
            ...
            self.storage
                .push(format!("    fillcolor=\"{GRAY_LIGHT}\";"));
        }
    }
    ...
    fn edge(&mut self, from: &str, to: &str, label: Option<&str>, style: RenderStyle) {
        let styling = if matches!(style, RenderStyle::Small) {
            ", style=dashed"
        } else {
            ""
        };
